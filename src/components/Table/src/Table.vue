  ...
  setup(props, { attrs, slots, emit, expose }) {
    ...
    const rnderTableColumn = (columnsChildren?: TableColumn[]) => {
      ...
      return [...[renderTableExpand()], ...[renderTableSelection()]].concat(
        (columnsChildren || columns)
          ...
          .map((v) => {
            const show = isBoolean(v.show) && !v.show
            // 自定生成序号
            if (v.type === 'index') {
    ...
    return () => (
      <div v-loading={unref(getProps).loading}>
        <Settings columns={unref(getProps).columns} onSetColumn={setColumn}></Settings>
        <ElTable
          // @ts-ignore
          ref={elTableRef}
